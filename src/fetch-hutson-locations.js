  ...
  return new Promise((resolve, reject) => {
    Lob.addresses.list({ limit: 100, ...options }, (err, res) => {
      ...
      if (res.next_url) {
        ...
        fetchLobAddresses(helpers, { after: searchParams.get('after') }, addresses)
          .then(res => {
            resolve(res)
          })
          .catch(err => {
...
const fetchHutsonLocations = async helpers => {
  ...
  const lobHutsonAddresses = await fetchLobAddresses(helpers)
    .then(res => res.filter(obj => obj.company.toUpperCase() === 'HUTSON INC'))
    .catch(err => {
      console.error('There was a problem fetching addresses from Lob: ', err)
      process.exit(1)
    })
  ...
  const sanityHutsonLocations = await client
    ...
    .catch(err => {
      console.error('There was a problem fetching Hutson location data from Santiy: ', err)
      process.exit(1)
    })
    ...
    sanityHutsonLocations.map(loc => {
      return new Promise((resolve, reject) => {
        const match = lobHutsonAddresses.find(addr => {
          return String(addr.address_zip).slice(0, 5) === String(loc.zip).slice(0, 5)
        })
        if (!match) {
          ...
          createLobAddress(helpers, {
            ...
            .catch(err => {
              reject('Failed to create a new Lob address for location: ', loc.title)
            })
        } else {
