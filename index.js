...
const { scrubEmail, scrubState, scrubZip } = require('@hutsoninc/data-scrubber')
const {
  getMonthName,
  isDefined,
  isEmptyString,
...
const equipmentLookupFilePath = path.join(__dirname, `src/data/${equipmentLookupFileName}`)
...
const validateProperties = (data, propertiesArr, { fileName }) => {
  ...
  propertiesArr.forEach(prop => {
    if (!props.includes(prop)) {
      throw new Error(`Property ${prop} could not be found in file ${fileName}`)
    }
  })
}
  ...
  return data.reduce((acc, obj) => {
    if (test(obj)) {
      ...
      const matchIndex = acc.findIndex(
        i => `${i.street1}-${i.postalCode}` === `${obj.street1}-${obj.postalCode}`
      )
      if (matchIndex === -1) {
        acc.push({
          ...rest,
...
const main = async () => {
  ...
  const helpers = {
    lobLimiter,
    Lob: lob(process.env[promptResponses.dryRun ? 'TEST_LOB_KEY' : 'LIVE_LOB_KEY']),
    promptResponses,
  }
  ...
  const equipmentLookupFileContents = fs.readFileSync(equipmentLookupFilePath, 'utf-8')
  const equipmentLookup = await csvToJson(equipmentLookupFileContents)

  // Get templates
  const agCceLetterTemplate = fs.readFileSync(agCceLetterTemplatePath, 'utf-8')
  ...
  const combinedData = spreadsheetFilesJson.reduce((acc, dataset) => {
    dataset.data.forEach(machine => {
      const matchIndex = acc.data.findIndex(obj => obj.PIN === machine['PIN-17'])
      if (matchIndex !== -1) {
  ...
  console.log(`Found ${combinedData.data.length} records. Cleaning up and filtering data...`)
  ...
  const data = combinedData.data
    .map(obj => {
      ...
      const match = equipmentLookup.find(row => pin.indexOf(row.id) === 0)
      if (!match) {
      ...
      return {
        ...
        name: isEmptyString(name) || name.length > 40 ? 'Current Resident' : name,
      ...
      return new Promise(resolve => {
        ...
        lobLimiter.schedule(() => {
          const data = {
            ...
            merge_variables: {
              hutsonAddressLine1: obj.hutsonLocationDetails.address,
              ...
              addressLine1: [obj.street1, obj.street2]
                .filter(val => !isEmptyString(val) && isDefined(val))
                .join(', '),
              ...
              equipmentList: obj.models
                ...
                .join(''),
              hutsonPhoneNumber: obj.hutsonLocationDetails.phoneNumber,
              date: dateString,
            },
            color: true,
  ...
  const powerGardEmailList = data
    .reduce((acc, obj) => {
      if (obj.pgEligible && isDefined(obj.email) && !isEmptyString(obj.email)) {
    ...
    .map(obj => {
      return {
        email: obj.email,
        warranty_expiration_details: toListString(obj.warranty_expiration_details, {
          delimiter: ';',
        }),
        warranty_expiration_model: toListString(obj.warranty_expiration_model),
      }
    })

  const csvHeaders = Object.keys(powerGardEmailList[0])

  const powerGardEmailListCsv = await jsonToCsv(powerGardEmailList, csvHeaders)
    ...
    await Promise.all(
      spreadsheetFiles.map(fileName => {
        return fs.move(
          ...
          path.join(uploadsCompletedPath, `${runningForMonthName} ${runningForYear}`, fileName)
        )
      })
    )
  }
